  ...
  initOperation(state, action, dispatch) {
    ...
    if (!payload.iss) {
      throw new InvalidJwtPayloadError(
        "JWT payload missing issuer (iss field)"
      );
    }
    ...
    if (!vc) {
      throw new InvalidJwtPayloadError(
        "JWT payload does not contain a verifiable credential (vc field)"
      );
    }
    ...
    // Context
    if (vc["@context"]) {
      const context = Array.isArray(vc["@context"])
        ? vc["@context"]
        : [vc["@context"]];
    ...
    if (vc.credentialSubject !== undefined) {
      const credentialSubjectStr =
        typeof vc.credentialSubject === "string"
